...
import { DEFAULT_PROFILE, contextTokensPerTurn, POOL } from '../core/pipeline.js';
...
// Quota cell, e.g. "$30", "Unlimited", or a promo "$15 $60 4x · Ends Sep 20".
// Take the FIRST $-amount (the standing base limit; a promo like "4x · Ends Sep 20"
// is a temporary annotation we do not model). Never strip separators and concatenate
// every digit — "$15 $60" must not become 15604.
function parseUsage(s) {
  const m = String(s).match(/\$\s*([\d,]+(?:\.\d+)?)/);
  if (!m) return 0; // "Unlimited" / "Free" / "-"
...
function parseGoTable(html) {
  ...
  // Parse usage as a number (see parseUsage: handles "$30" and promo cells).
  for (const r of rows) {
    r.usage = parseUsage(r.usage);
  }
