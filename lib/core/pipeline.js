...
// Measured session profile (method + sample: docs/profiles/measured.json).
// One "task" is a full autonomous root session, NOT a single turn: cache reuse and the
// growing context only emerge across the turns, so the cost is turns × per-turn buckets.
// Formerly a fixed 7,750 input + 147,250 cache-read + 300 output "session" (which was
// really one steady-state turn, and had no source). Measured (median root session):
//   turns      = 147
//   input      = 5,649  fresh tokens / turn
//   cache_read = 120,424 cache-served tokens / turn (94.9% of processed tokens)
//   cache_write=   137 tokens written to cache / turn
//   output     =   728 tokens / turn (output + reasoning — both bill at the output rate)
const PROFILES = {
  agentic: {
    name: 'agentic',
    turns: 147,
...
const DEFAULT_PROFILE = PROFILES.agentic;

const POOL = 60; // virtual monthly quota pool in USD — OpenCode's accounting unit, NOT the price
// What the card is actually charged per month (author-reported, Sept 2026). Not scraped from the
// price page: the page only ever shows quotas/multipliers. Kept here so the report can state the
// real charge next to the virtual pool — mp is a share of the pool, i.e. the same % of this charge.
const SUBSCRIPTION_USD = 10;

function sessionPrice(go, profile = DEFAULT_PROFILE) {
  const p = profile.perTurn;
  return (
    (p.input     * go.inputPerM     +
     p.cacheRead * go.cacheReadPerM +
     p.cacheWrite * go.cacheWritePerM +
     p.output    * go.outputPerM) / 1_000_000
  ) * profile.turns;
}

// Tokens processed per turn = the context size at a mid-session turn.
function contextTokensPerTurn(profile = DEFAULT_PROFILE) {
  const p = profile.perTurn;
  return p.input + p.cacheRead + p.cacheWrite + p.output;
}

// Cost decomposition of one session: which token class actually drives the bill.
// This is the project's thesis made per-row: a tariff can look cheap on input/output
// while its cache-read price (94.9% of the tokens) quietly dominates a long session.
function sessionCostSplit(go, profile = DEFAULT_PROFILE) {
  const p = profile.perTurn;
  const parts = {
    input:      p.input      * go.inputPerM,
    cacheRead:  p.cacheRead  * go.cacheReadPerM,
    cacheWrite: p.cacheWrite * go.cacheWritePerM,
    output:     p.output     * go.outputPerM,
  };
  const total = parts.input + parts.cacheRead + parts.cacheWrite + parts.output;
  if (!total) return null;
  return {
    input: parts.input / total,
    cacheRead: parts.cacheRead / total,
    cacheWrite: parts.cacheWrite / total,
    output: parts.output / total,
  };
}
...
function buildReport(goRows, pool, latestSlug) {
  ...
  const rows = joined.map(({ go, bench, issue, sim, tier }) => {
    ...
    // Wallet view — how far the ACTUAL monthly charge stretches in list-price work.
    // This is the number a subscriber cares about; `multiplier` above is the vendor's scale.
    const walletMultiplier = go.usage ? go.usage / SUBSCRIPTION_USD : null;
    const sessionsPerMonth = go.usage ? Math.floor(go.usage / price) : null;
    return {
      model: go.model,
      ...
      walletMultiplier,
      sessionsPerMonth,
      inputPerM: go.inputPerM,
      outputPerM: go.outputPerM,
      cacheReadPerM: go.cacheReadPerM,
      cacheWritePerM: go.cacheWritePerM,
      costSplit: sessionCostSplit(go),
...
function reportToJson(report, sources, siteUrl) {
  return {
    ...
    session: {
      ...
      turns: DEFAULT_PROFILE.turns,
      perTurn: {
        inputTokens: DEFAULT_PROFILE.perTurn.input,
        cacheReadTokens: DEFAULT_PROFILE.perTurn.cacheRead,
        cacheWriteTokens: DEFAULT_PROFILE.perTurn.cacheWrite,
        outputTokens: DEFAULT_PROFILE.perTurn.output,
      },
      contextTokensPerTurn: contextTokensPerTurn(DEFAULT_PROFILE),
      cacheReadShare: round(DEFAULT_PROFILE.perTurn.cacheRead / contextTokensPerTurn(DEFAULT_PROFILE), 4),
      monthlySubscriptionUsd: SUBSCRIPTION_USD,
      monthlyPoolUsd: POOL,
    },
    rows: report.rows.map((r) => ({
      ...
      walletMultiplier: r.walletMultiplier == null ? null : round(r.walletMultiplier, 4),
      ...
      costSplit: r.costSplit
        ? {
            input: round(r.costSplit.input, 4),
            cacheRead: round(r.costSplit.cacheRead, 4),
            cacheWrite: round(r.costSplit.cacheWrite, 4),
            output: round(r.costSplit.output, 4),
          }
        : null,
...
function loadPrevSnapshot(outDir, excludeDate = null) {
  ...
  // A same-day re-run must diff against the previous DAY, never against the snapshot
  // this run is about to rewrite — otherwise the second run of the day computes a
  // self-diff (0 changes vs itself) and erases the day's real change events.
  const last = excludeDate
    ? [...list].reverse().find((s) => s.date !== excludeDate)
    : list[list.length - 1];
  if (!last) return null;
...
function writeSnapshot(outDir, full, changesCount) {
  ...
  const entry = {
    ...
    best: top ? `${top.model} · ${top.scorePerMp.toFixed(3)}` : null,
